...
- (void)createViews
{
    ...
    self.toolbar.backgroundColor = [self colorWithHexString:_browserOptions.navbarcolor];
    
    self.backButton = [UIButton buttonWithType:UIButtonTypeCustom];
    self.backButton.enabled = YES;
    [self.backButton setFrame:CGRectMake(0, 0, 34, TOOLBAR_HEIGHT)];
    [self.backButton addTarget:self action:@selector(goBack:) forControlEvents:UIControlEventTouchUpInside];
    [self.backButton setImage:[UIImage imageNamed:@"in_app_browser_back_btn"] forState:UIControlStateNormal];
    [self.backButton setImage:[UIImage imageNamed:@"in_app_browser_back_btn"] forState:UIControlStateNormal];
    [self.backButton setImageEdgeInsets:UIEdgeInsetsMake(12, 11, 12, 11)];
    [self.toolbar addSubview:self.backButton];
    
    UIView *splitView = [[UIView alloc] init];
    [splitView setFrame:CGRectMake(self.backButton.frame.size.width + 5, (TOOLBAR_HEIGHT - 15) / 2, 1, 15)];
    [splitView setBackgroundColor:[UIColor colorWithWhite:255 alpha:0.5]];
    [self.toolbar addSubview:splitView];
    
    self.closeButton = [UIButton buttonWithType:UIButtonTypeCustom];
    self.closeButton.enabled = YES;
    [self.closeButton setFrame:CGRectMake(self.backButton.frame.size.width + 5 + splitView.frame.size.width, 0, 55, TOOLBAR_HEIGHT)];
    ...
    [self.closeButton.titleLabel setFont:[UIFont systemFontOfSize:15.0]];
    [self.closeButton addTarget:self action:@selector(close:) forControlEvents:UIControlEventTouchUpInside];
    [self.toolbar addSubview:self.closeButton];
    
    self.titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.toolbar.frame.size.width, TOOLBAR_HEIGHT)];
    self.titleLabel.textAlignment = NSTextAlignmentCenter;
    self.titleLabel.numberOfLines = 1;
    self.titleLabel.lineBreakMode = NSLineBreakByTruncatingTail;
    self.titleLabel.textColor = [UIColor whiteColor];
    [self.titleLabel setFont:[UIFont systemFontOfSize:17.0]];
    [self setTitleText:_browserOptions.title];
    ...
    statusBarView.backgroundColor = [self colorWithHexString:_browserOptions.navbarcolor];
...
- (void)setTitleText:(NSString *)title {
    if (title && title.length > 0) {
        if (title.length > 7) {
            self.titleLabel.text = [[title substringToIndex:7] stringByAppendingString:@".."];
        } else {
...
- (void)webViewDidFinishLoad:(UIWebView*)theWebView
{
    ...
    if (self.titleLabel && _browserOptions.title.length == 0) {
        // Update title text to page title when title is shown and we are not
        // required to show a static text.
        [self setTitleText:[self.webView stringByEvaluatingJavaScriptFromString:@"document.title"]];
    }
}
...
- (id)init
{
    if (self = [super init]) {
        ...
        self.location = YES;
        self.toolbar = YES;
        self.closebuttoncaption = @"关闭";
        self.navbarcolor = @"#000000";
        self.title = @"";
